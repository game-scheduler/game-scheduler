...
- `.github/workflows/ci-cd.yml` — replaced with complete new workflow; 10 jobs: `code-quality`, `unit-tests`, `diff-coverage`, `frontend-tests`, `integration-tests`, `check-suppressions`, `suppression-gate`, `jscpd-check`, `docker-build`, `build-and-publish`; `retry` added to both Docker matrix definitions; prerequisite comment at top of file (Tasks 2.1, 2.2)
- `docs/developer/ci-cd.md` — new file documenting the pipeline jobs and the manual `lint-suppression-review` environment setup steps (Task 2.2)
